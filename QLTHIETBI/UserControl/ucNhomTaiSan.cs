...
namespace QLTHIETBI
{
    public partial class ucNhomTaiSan : UserControl
    {
        BindingSource nhomtsList = new BindingSource();
        ...
        {
            nhomtsList.DataSource = NhomTaiSanDAO.Instance.GetDataNhomTaiSan(page);
            dgvNhomTS.DataSource = nhomtsList;
            txtPage.Text = page.ToString();
        }
        void LoadCombobox()
        {
            cbxSearch.Items.Add("Mã nhóm tài sản");
        ...
        void AddFoodBinding()
        {
            lblTittle.DataBindings.Add(new Binding("Text", dgvNhomTS.DataSource, "MANHOMTS", true, DataSourceUpdateMode.Never));
            txtTenNTS.DataBindings.Add(new Binding("Text", dgvNhomTS.DataSource, "TENNHOMTS", true, DataSourceUpdateMode.Never));
        }
        #endregion
        ...
        {
            if (PhanQuyenDAO.Instance.GetChiTietQuyen(TaikhoanObj.Username, "Nhóm Tài Sản").Rows[0][1].ToString() == "True")
            {
                HoatDongObj.Noidung = "Thêm";
                lblTittle.Text = funtions.SDienMaTuDong("NTS");
                CLeanTextBox(txtTenNTS);
            }
        ...
        {
            if (!string.IsNullOrEmpty(txtTenNTS.Text))
            {
                switch (HoatDongObj.Noidung)
                {
                    case "Thêm":
                        if (NhomTaiSanDAO.Instance.Them(lblTittle.Text, txtTenNTS.Text))
                        {
                            LichSuHoatDongDAO.Instance.ThongBao(1, lblTittle.Text);
                            LoadData(Convert.ToInt32(txtPage.Text));
                            txtTenNTS.Enabled = false;
                    ...
                    case "Sửa":
                        if (NhomTaiSanDAO.Instance.Sua(lblTittle.Text, txtTenNTS.Text))
                        {
        ...
        private void dgvNhomTS_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            switch (e.ColumnIndex)
            {
                case 0:
                    if (PhanQuyenDAO.Instance.GetChiTietQuyen(TaikhoanObj.Username, "Nhóm Tài Sản").Rows[0][2].ToString() == "True")
                    {
                        HoatDongObj.Noidung = "Sửa";
                        txtTenNTS.Enabled = true;
                    }
                    ...
                    break;
                case 1:
                    if (PhanQuyenDAO.Instance.GetChiTietQuyen(TaikhoanObj.Username, "Nhóm Tài Sản").Rows[0][3].ToString() == "True")
                    {
                        ...
                        {
                            if (NhomTaiSanDAO.Instance.Xoa(lblTittle.Text))
                            {
        ...
        {
            int count = NhomTaiSanDAO.Instance.CountDataNhomTaiSan();
        ...
        {
            int page = Convert.ToInt32(txtPage.Text);
            int count = NhomTaiSanDAO.Instance.CountDataNhomTaiSan() / 10;
            if (count % 10 != 0)
                count++;
            else count = 1;
        ...
        private void txtSearch_OnIconRightClick(object sender, EventArgs e)
        {
            ...
            {
                case 0:
                    dt = NhomTaiSanDAO.Instance.TimKiemTheoTen("MANHOMTS", txtSearch.Text);
                    break;
                case 1:
                    dt = NhomTaiSanDAO.Instance.TimKiemTheoTen("TENNHOMTS", txtSearch.Text);
                    break;
            }

            if (dt != null && dt.Rows.Count > 0 && !string.IsNullOrEmpty(txtSearch.Text))
            {
                nhomtsList.DataSource = dt;
                dgvNhomTS.DataSource = nhomtsList;
            }
